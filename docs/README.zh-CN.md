...
- 自动回退：`自动` 模式下，未配置 API 密钥时会保持本地工作流；若 LLM 请求失败，也会自动回退到非 LLM 流程。
...
2. 可选的本地神经网络重排序
   ...
   - 可在设置中通过 `下载模型`、`编译模型`、`重新编译模型`、`删除模型` 管理模型。
3. 可选的 LLM 高亮
   - 需要配置 `API Key`、`Base URL` 和 `Model`。
   - 适合希望优先使用大模型判断高亮内容的场景。

`高亮后端` 用来决定高亮是走 LLM 路径，还是走本地非 LLM 路径。神经网络重排序属于本地非 LLM 路径上的额外增强能力，需要在设置中单独启用和管理：

- `自动`：推荐默认值；未配置 API 密钥时优先走本地流程，LLM 失败时也会安全回退。
- `优先 LLM`：优先使用已配置的 LLM 路径。
- `仅非 LLM`：仅使用本地非 LLM 排序流程。
...
- `高亮后端`：用于选择优先走 LLM 路径还是本地非 LLM 路径；神经网络重排序需单独控制。可选 `自动`、`优先 LLM`、`仅非 LLM`。
- `API 密钥`、`API 地址`、`模型`：仅在使用 LLM 高亮时需要配置。
...
- `阅读关注点`：可在 `平衡`、`结果与证据`、`方法与设计`、`局限与注意事项` 之间调整优先关注的内容。
- `神经网络重排序`：包含启用开关、运行状态，以及 `下载模型`、`编译模型`、`重新编译模型`、`删除模型` 等模型管理操作。

## 隐私与平台说明

- 内置的非 LLM 排序完全本地运行，不需要 API 密钥。
